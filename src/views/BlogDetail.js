import { StyleSheet, Text, View, useWindowDimensions, ScrollView, Image } from "react-native";
...
const BlogDetail = ({ route, navigation }) => {
  ...
  const getPost = async () => {
    try {
      const response = await fetch(`https://public-api.wordpress.com/rest/v1.1/sites/martinmelodev.wordpress.com/posts/${blogId}`);
  ...
  const blogClassStyles = {
    blTile: { marginLeft: "auto", marginRight: "auto" },
